...
<Tip>
For live previews while the model streams tool arguments (e.g. in MCP Inspector or MCP Apps clients), see [Streaming tool arguments](/typescript/server/widget-components/usewidget#streaming-tool-arguments) in the useWidget docs.
</Tip>
...
See [useCallTool()](/typescript/server/widget-components/usecalltool) for more patterns including callbacks, async/await, and error handling.
