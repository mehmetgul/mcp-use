...
Demonstrates **streaming tool arguments** to widgets: when an LLM streams tool call arguments, the widget receives partial input in real time via `partialToolInput` and `isStreaming` from [useWidget](/typescript/server/widget-components/usewidget#streaming-tool-arguments), enabling live code or JSON previews as the model generates them.

**Example location:** `libraries/typescript/packages/mcp-use/examples/server/features/streaming-props`

**Run from package root (mcp-use package):**
